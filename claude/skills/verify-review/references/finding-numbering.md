...
- **検証対象の既存レビューコメント**: レビューコメント内での**出現順**で採番する。verify-review の主目的は妥当性検証であり、重要度の裁定は責務外であるため。
- **自分で起票する Finding**（検証過程で新たに発見した問題）: **重要度の降順**（Must Fix → Should Fix → Nit）で採番する。同一重要度内の順序は任意。
