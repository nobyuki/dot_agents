...
3 チケットを登録し、元チケットとのリンク作成 + PROJ-101 に「AI分割済み」ラベルを付与します。
「登録して」で実行します。
```
...
5. **issue link を作成** — 各子チケットと元チケットの間にリンクを作成:
   - `getIssueLinkTypes` でリンクタイプ一覧を取得（初回のみ）
   - リンクタイプを以下の優先順で選択:
     1. `Work item split` があればそれを使う
     2. なければ `Relates` を探して使う
     3. どちらもなければ link 作成はスキップし、手動でリンクを追加するよう案内を表示する
   - `Work item split` の場合:
     - `outwardIssue`: 元チケット（split to の方向）
     - `inwardIssue`: 子チケット（split from の方向）
   - `Relates` にフォールバックした場合:
     - 結果サマリーに「split 専用 link type がないため汎用リンクで代替」と明記する
   - `createIssueLink` が失敗した場合:
     - 子チケット作成自体は成功扱いで継続する（link 作成は補助的処理）
     - 失敗したリンクを警告として結果サマリーに記録し、手動追加を案内する
...
| 子チケット | タイトル | リンク | URL |
|-----------|--------|-------|-----|
| PROJ-201 | API実装 | ✅ Work item split | ... |
| PROJ-202 | UI実装 | ✅ Work item split | ... |
| PROJ-203 | テスト追加 | ⚠️ リンク作成失敗（手動追加してください） | ... |
...
| 2 | PROJ-102 | ⏭️ 分割不要 | — |
| 3 | PROJ-103 | ⚠️ 分割完了（3件、リンク作成失敗1件） | PROJ-204, PROJ-205, PROJ-206 |
```
